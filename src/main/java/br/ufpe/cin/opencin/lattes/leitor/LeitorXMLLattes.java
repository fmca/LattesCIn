	...
	public static Professor lerXML(File f, List<Professor> professoresID)
		...
		while(i.hasNext()){
			...
			if(partes.getName().equals("DADOS-GERAIS")){
				String nomeProfessor = partes.getAttributeValue("NOME-COMPLETO");
				professor = procurarProfessorPorNome(nomeProfessor, professoresID);
				professor.setNomeCitacoes(partes.getAttributeValue("NOME-EM-CITACOES-BIBLIOGRAFICAS"));
				professor.setBirthDate(partes.getAttributeValue("PAIS-DE-NASCIMENTO"));
				professor.setNacionalidade(partes.getAttributeValue("NACIONALIDADE"));
				professor.setPaisNascimento(partes.getAttributeValue("PAIS-DE-NASCIMENTO"));
				professor.setUFNascimento(partes.getAttributeValue("UF-NASCIMENTO"));
				professor.setCidadeNascimento(partes.getAttributeValue("CIDADE-NASCIMENTO"));
			}


			//Pega as producoes bibliograficas
			if(partes.getName().equals("PRODUCAO-BIBLIOGRAFICA")){
				List<Element> producoesBibliograficas = partes.getChildren();
